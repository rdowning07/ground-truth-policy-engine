...
**See also:** [Python overview: Day 1](python_overview_day1.md) for full “what / why / how” and the **Quick reference** table for concept → Python syntax.
...
| Term | Definition | See |
|------|------------|-----|
| **accumulator** | A variable (often a list) you keep outside a loop and update inside it to collect results (e.g. `results = []` then `results.append(...)`). | Day 1 Stage 10; Phase 1 |
| **block** | A group of statements that belong together, marked by indentation (e.g. the body of an `if` or `for`). | Day 1 Stages 6, 9 |
| **boolean** | A value that is either `True` or `False`; used for yes/no, pass/fail. | Day 1 Stage 2 |
| **comment** | A line (or part of a line) starting with `#`; Python ignores it. Used to explain code to humans. | Day 1 Stage 11 |
| **comparison** | An expression that evaluates to `True` or `False` (e.g. `age >= 18`, `x == y`). | Day 1 Stage 5 |
| **conditional** | A branch in the program: “if this condition is true, do this; else do that” (`if` / `else`). | Day 1 Stage 6 |
| **dict** | A mapping from keys to values; used for one record with named fields (e.g. `{"id": 1, "age": 25}`). | Day 1 Stage 8 |
| **deterministic** | Same inputs always produce the same outputs; no randomness or hidden state that changes the result. | Day 1 Stage 4; Phase 1 |
| **docstring** | A triple-quoted string right under `def ...` that describes what the function does. | Day 1 Stage 11 |
| **function** | A named block of code that takes parameters and can return a value; defined with `def name(...):`. | Day 1 Stage 11 |
| **indentation** | In Python, the spaces at the start of a line define which block the line belongs to; wrong indentation causes errors or wrong behavior. | Day 1 Stage 6 |
| **list** | An ordered collection of values, written with `[...]`; we use it for “many records.” | Day 1 Stage 7 |
| **loop (for)** | Code that runs once for each item in a collection (`for item in my_list:`). | Day 1 Stage 9 |
| **None** | Python’s value meaning “no value” or “not set”; used for optional fields (e.g. no failure reason). | Day 1 Stage 2; Phase 2+ |
| **parameter** | A name in the function definition that receives an argument when the function is called (e.g. `records` in `def evaluate_age_rule(records):`). | Day 1 Stage 11 |
| **record** | In this curriculum, one item (e.g. one user) represented as a dict with named fields (id, age, etc.). | Day 1 Stage 8; Phase 1 |
| **script** | A text file with a `.py` extension that Python can run from top to bottom (e.g. `python3 src/engine.py`). | Day 1 Stage 1 |
| **type** | The kind of a value: number (int/float), string, boolean, list, dict, etc. Python infers types; you don’t declare them. | Day 1 Stage 2 |
| **value** | A single piece of data Python can work with (e.g. `18`, `"US"`, `True`). | Day 1 Stage 2 |
| **variable** | A name that refers to a value; created by `name = value`. | Day 1 Stage 3 |
...
| **KeyError** | The error Python raises when you use `record["key"]` and that key is not in the dict. | Day 1 Stage 8; Phase 2 |
...
| Term | Definition | See |
|------|------------|-----|
| **pure (rule/function)** | No side effects: no mutation of shared state, no printing, no I/O inside the rule; same inputs → same return value. | Day 1 Stage 4; Charter |
...
Back to [Curriculum overview](curriculum_overview.md) · [Python overview: Day 1](python_overview_day1.md)
